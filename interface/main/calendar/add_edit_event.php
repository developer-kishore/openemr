    ...
    function DOBandEncounter($pc_eid)
    {
        ...
        if (!empty($_POST['form_pid'])) {
            ...
            if ((int)$GLOBALS['auto_create_new_encounters']
                ...
                && $is_checkin == '1'
                && !$is_tracker) {
                $encounter = todaysEncounterCheck($_POST['form_pid'], $event_date, $_POST['form_report'], $_POST['form_comments'], $_POST['facility'], $_POST['billing_facility'], $_POST['form_provider'], $_POST['form_category'], false);
                if ($encounter) {
        ...
        if (!empty($_POST['form_gid'])) {
            ...
            if ($GLOBALS['auto_create_new_encounters'] && $event_date == date('Y-m-d') && $_POST['form_apptstatus'] == '=') {
                $encounter = todaysTherapyGroupEncounterCheck($_POST['form_gid'], $event_date, $_POST['form_report'], $_POST['form_comments'], $_POST['facility'], $_POST['billing_facility'], $_POST['form_provider'], $_POST['form_category'], false, $pc_eid);
                if ($encounter) {
...
<tr id="days_every_week_row">
    ...
    <td><b><?php echo xlt('Report'); ?></b></td>
    <td colspan='1' nowrap>
   <?php 
   echo generate_select_list('form_report', 'Report', $report, xl('Report'));
   ?>        
    </td>
