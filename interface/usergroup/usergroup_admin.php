...
if (isset($_POST["privatemode"]) && $_POST["privatemode"] =="user_admin") {
    if ($_POST["mode"] == "update") {
        ...
        if ($_POST["report"]) {
            sqlStatement("update users set report=? where id= ? ", array($_POST["report"], $_POST["id"]));
        }
        if ($_POST["fname"]) {
    ...
    if ($_POST["mode"] == "new_user") {
        ...
        if ($doit == true) {
          ...
          //if password expiration option is enabled,  calculate the expiration date of the password
            ...
            "', report = '"         . add_escape_custom(trim((isset($_POST['report']) ? $_POST['report'] : ''))) .
